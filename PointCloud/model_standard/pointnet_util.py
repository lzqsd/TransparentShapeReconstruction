...
def sample_and_group(npoint, radius, nsample, xyz, nxyz, points ):
    ...
    if points is not None:
        ...
        new_points = torch.cat([grouped_xyz_norm * 2, grouped_points], dim=-1) # [B, npoint, nsample, C+D]
    else:
        new_points = torch.cat([grouped_xyz_norm * 2], dim=-1)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, nxyz1, xyz2, nxyz2, points1, points2):
        ...
        if points1 is not None:
            ...
            new_points = torch.cat([points1, 2*xyz_diff, interpolated_points], dim=-1 )
        else:
            new_points = torch.cat([2*xyz_diff, interpolated_points ], dim=-1 )
